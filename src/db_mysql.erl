...
init([]) ->
    ...
    {ok,_} = p1_mysql:start_link(conn,zt_misc:get_env(mysql_host),
                                    zt_misc:get_env(mysql_port),
                                    zt_misc:get_env(mysql_user),
                                    zt_misc:get_env(mysql_password),
                                    zt_misc:get_env(mysql_database)),
    {ok, #state{}}.
...
handle_cast({insert_ic,Code,Num,IC5,IC6,IC9,OPT,Point1,Point2,Point3}, State)  ->
    Q2 = io_lib:format("insert into zt_ic(iindex,inum,month05,month06,month09,op,point1,point2,point3) values
                      ('~s','~s','~s','~s','~s','~s','~s','~s','~s')",
    [Code,integer_to_binary(Num),integer_to_binary(IC5),integer_to_binary(IC6),
    integer_to_binary(IC9),integer_to_binary(OPT),integer_to_binary(Point1),integer_to_binary(Point2),integer_to_binary(Point3)]),
    p1_mysql:fetch(conn,Q2),
    {noreply, State};

handle_cast({insert_ih,Code,Num,IC5,IC6,IC9,OPT,Point1,Point2,Point3}, State)  ->
    Q2 = io_lib:format("insert into zt_ih(iindex,inum,month05,month06,month09,op,point1,point2,point3) values
                      ('~s','~s','~s','~s','~s','~s','~s','~s','~s')",
    [Code,integer_to_binary(Num),integer_to_binary(IC5),integer_to_binary(IC6),
    integer_to_binary(IC9),integer_to_binary(OPT),integer_to_binary(Point1),integer_to_binary(Point2),integer_to_binary(Point3)]),
    p1_mysql:fetch(conn,Q2),
...
handle_cast({insert_if,Code,Num,IC5,IC6,IC9,OPT,Point1,Point2,Point3}, State)  ->
    Q2 = io_lib:format("insert into zt_if(iindex,inum,month05,month06,month09,op,point1,point2,point3) values
                      ('~s','~s','~s','~s','~s','~s','~s','~s','~s')",
    [Code,integer_to_binary(Num),integer_to_binary(IC5),integer_to_binary(IC6),
    integer_to_binary(IC9),integer_to_binary(OPT),integer_to_binary(Point1),integer_to_binary(Point2),integer_to_binary(Point3)]),
    p1_mysql:fetch(conn,Q2),
