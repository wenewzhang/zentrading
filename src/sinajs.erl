...
init([]) ->
    ...
    % {State} = check_the_last_data(Name,#index{}),
    {ok, #index{}}.
...
handle_cast({get_the_last_data,Code,Pid}, Index) when is_list(Code) ->
    {IndexN} = check_the_last_data(Code,Index),
    gen_server:cast(Pid,{return_last_index,IndexN}),
    {noreply, IndexN};
...
check_the_last_data(Code,State) ->
  ...
  {ok,{_,_,HttpData}} ->
    ...
    Price  = lists:nth(4,ReData),
    ?LOG(httpc_request_user_success,[HttpData]),
    {State#index{code = Code,price = list_to_float(Price)}};
  {error, Reasonx} ->
...
%%% Internal functions
%%%===================================================================
build_access_url(Name) when is_atom(Name) ->
  Num = rand:uniform(10000000000),
  ?ZZ500_URL ++integer_to_list(Num) ++ "&list=" ++ atom_to_list(Name);
build_access_url(Name)  ->
  Num = rand:uniform(10000000000),
  ?ZZ500_URL ++integer_to_list(Num) ++ "&list=" ++ Name.
