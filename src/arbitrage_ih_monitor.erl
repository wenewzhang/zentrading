...
init([]) ->
    ...
    Fic1   = #future{code = "CFFEXIH1705",name = "IH1705",price = 0,old_p = 0,days = 1},
    Fic2   = #future{code = "CFFEXIH1706",name = "IH1706",price = 0,old_p = 0,days = 2},
    Fic3   = #future{code = "CFFEXIH1709",name = "IH1709",price = 0,old_p = 0,days = 5},
    Index  = #index{code = "sh000016",price = 0,old_p = 0},
    State0 = #state{fic1 = Fic1,fic2 = Fic2,fic3 = Fic3,index = Index,count = 0},
    State1 = qp_event:init_stats_timer(State0, #state.tm,?IH_MONITOR_INTERVAL*1000),
...
handle_cast({return_last_data,#future{code = Code,price = Price}},
                            #state{fic1 = #future{price = OldPrice} = Fic1,count = Count} = State) when Code =:= "CFFEXIH1705" ->
...
handle_cast({return_last_data,#future{code = Code,price = Price}},
                            #state{fic2 = #future{price = OldPrice} = Fic2,count = Count} = State) when Code =:= "CFFEXIH1706" ->
    io:format("------------------------------------------------------------------------------------------------------------股指期货:~p 现：~p 原：~p ~n",[Code,Price,OldPrice]),
    if Price > 0 ->
...
handle_cast({return_last_data,#future{code = Code,price = Price}},
                            #state{fic3 = #future{price = OldPrice} = Fic3,count = Count} = State) when Code =:= "CFFEXIH1709" ->
    io:format("------------------------------------------------------------------------------------------------------------股指期货:~p 现：~p 原：~p ~n",[Code,Price,OldPrice]),
    if Price > 0 ->
...
handle_cast({return_last_index,#index{code = Code,price = Price}},
    ...
    io:format("~n------------------------------------------------------------------------------------------------------------上证50指数:~p 现：~p 原：~p ~n",[Code,Price,OldPrice]),
    if Price > 0 ->
...
handle_info(monitor_timer, #state{fic1  = #future{code = Code1},
    ...
    gen_server:cast(future_current_month_ih,{get_the_last_data,Code1,self()}),
    gen_server:cast(future_current_month_ih,{get_the_last_data,Code2,self()}),
    gen_server:cast(future_next_season_ih,{get_the_last_data,Code3,self()}),
    gen_server:cast(stockmarket_ih,{get_the_last_data,Index,self()}),
    {noreply, State};
...
one_round_finished(#state{count = Count,fic1  = #future{code = C1,price = P1,old_p = OP1,days = D1} ,
  ...
  if (P1 =:= OP1 andalso P2 =:= OP2 andalso PI =:= OPI) orelse PI =:= 0 orelse P1 =:= 0  orelse P2 =:= 0 orelse OPI =:= 0 ->
    ...
    true ->
      ...
      if abs(Point1) > ?IH_POINT_TO_NOTICE andalso  P1 > 0 andalso P2 > 0  ->
        OPT1 = true,
      ...
      if abs(Point2) > ?IH_POINT_TO_NOTICE andalso  P1 > 0 andalso P3 > 0  ->
        OPT2 = true,
        os:cmd("afplay ./zentrading-0.1.0/src/yi.mp3");
      true ->
        OPT2 = false
      ...
      if abs(Point3) > ?IH_POINT_TO_NOTICE andalso  P2 > 0 andalso P3 > 0 ->
        OPT3 = true,
        os:cmd("afplay ./zentrading-0.1.0/src/yi.mp3");
      true ->
        OPT3 = false
      ...
      if OPT1 orelse OPT2 orelse OPT3 ->
        gen_server:cast(db_mysql,{insert_ih,ICode,round(PI*1000),P1,P2,P3,1,Point1,Point2,Point3});
      true ->
        gen_server:cast(db_mysql,{insert_ih,ICode,round(PI*1000),P1,P2,P3,0,Point1,Point2,Point3})
      end,
      State2 = State#state{count = 0}
  end,
