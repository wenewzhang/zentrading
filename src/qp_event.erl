...
-export([start_link/0]).
-export([init_stats_timer/3, init_disabled_stats_timer/2,
...
-spec start_link() -> qp_types:ok_pid_or_error().
-spec init_stats_timer(container(), pos(),integer()) -> container().
...
init_stats_timer(C, P,Intval) ->
    % {ok, StatsLevel} = application:get_env(qp_auth, collect_statistics),
    % {ok, Interval}   = application:get_env(qp_auth, collect_statistics_interval),
    setelement(P, C, #state{level = undefined, interval = Intval,
                            timer = undefined}).
...
notify(Type, Props, Ref) ->
    io:format("notify in qp_event ~p ~p ~p ~n",[Type,Props,Ref]),
