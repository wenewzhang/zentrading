...
init([]) ->
    ...
    Fic1   = #future{code = "CFFEXIF1705",name = "IF1705",price = 0,old_p = 0,days = 1},
    Fic2   = #future{code = "CFFEXIF1706",name = "IF1706",price = 0,old_p = 0,days = 2},
    Fic3   = #future{code = "CFFEXIF1709",name = "IF1709",price = 0,old_p = 0,days = 5},
    Index  = #index{code = "sh000300",price = 0,old_p = 0},
    State0 = #state{fic1 = Fic1,fic2 = Fic2,fic3 = Fic3,index = Index,count = 0},
    State1 = qp_event:init_stats_timer(State0, #state.tm,?IF_MONITOR_INTERVAL*1000),
...
handle_cast({return_last_data,#future{code = Code,price = Price}},
                            #state{fic1 = #future{price = OldPrice} = Fic1,count = Count} = State) when Code =:= "CFFEXIF1705" ->
...
handle_cast({return_last_data,#future{code = Code,price = Price}},
                            #state{fic2 = #future{price = OldPrice} = Fic2,count = Count} = State) when Code =:= "CFFEXIF1706" ->
    io:format("----------------------------------------------------股指期货:~p 现：~p 原：~p ~n",[Code,Price,OldPrice]),
    if Price > 0 ->
...
handle_cast({return_last_data,#future{code = Code,price = Price}},
                            #state{fic3 = #future{price = OldPrice} = Fic3,count = Count} = State) when Code =:= "CFFEXIF1709" ->
    io:format("----------------------------------------------------股指期货:~p 现：~p 原：~p ~n",[Code,Price,OldPrice]),
    if Price > 0 ->
...
handle_cast({return_last_index,#index{code = Code,price = Price}},
    ...
    io:format("~n----------------------------------------------------沪深300指数:~p 现：~p 原：~p ~n",[Code,Price,OldPrice]),
    if Price > 0 ->
...
handle_info(monitor_timer, #state{fic1  = #future{code = Code1},
    ...
    gen_server:cast(future_current_month_if,{get_the_last_data,Code1,self()}),
    gen_server:cast(future_current_month_if,{get_the_last_data,Code2,self()}),
    gen_server:cast(future_next_season_if,{get_the_last_data,Code3,self()}),
    gen_server:cast(stockmarket_if,{get_the_last_data,Index,self()}),
    {noreply, State};
...
one_round_finished(#state{count = Count,fic1  = #future{code = C1,price = P1,old_p = OP1,days = D1} ,
  ...
  % if (P1 =:= OP1 andalso P2 =:= OP2 andalso PI =:= OPI) orelse OPI =:= 0 ->
  % if (P1 =:= OP1 andalso P2 =:= OP2 andalso PI =:= OPI) orelse OPI =:= 0 orelse P1 =:= 0  orelse P2 =:= 0  ->
  if (P1 =:= OP1 andalso P2 =:= OP2 andalso PI =:= OPI) orelse PI =:= 0 orelse P1 =:= 0  orelse P2 =:= 0 orelse OPI =:= 0 ->
    ...
    true ->
      ...
      if abs(Point1) > ?IF_POINT_TO_NOTICE andalso  P1 > 0 andalso P2 > 0  ->
        OPT1 = true,
      ...
      if abs(Point2) > ?IF_POINT_TO_NOTICE andalso  P1 > 0 andalso P3 > 0  ->
        OPT2 = true,
        os:cmd("afplay ./zentrading-0.1.0/src/yi.mp3");
      true ->
        OPT2 = false
      ...
      if abs(Point3) > ?IF_POINT_TO_NOTICE andalso  P2 > 0 andalso P3 > 0 ->
        OPT3 = true,
        os:cmd("afplay ./zentrading-0.1.0/src/yi.mp3");
      true ->
        OPT3 = false
      ...
      if OPT1 orelse OPT2 orelse OPT3 ->
        gen_server:cast(db_mysql,{insert_if,ICode,round(PI*1000),P1,P2,P3,1,Point1,Point2,Point3});
      true ->
        gen_server:cast(db_mysql,{insert_if,ICode,round(PI*1000),P1,P2,P3,0,Point1,Point2,Point3})
      end,
      State2 = State#state{count = 0}
  end,
  StateN = qp_event:init_stats_timer(State2, #state.tm,?IH_MONITOR_INTERVAL*1000),
